...
namespace Konvolucio.MTOO180701.UnitTest
{
    ...
    public class UnitTest_Test
    {
        ...
        [Test]
        public void SeriesRcImpedance()
        {
            double cap = 1E-6; // 100nF
            double f = 1000;     // 1KHz
            ...
            double impRe = 1E+3;
            double impIm = xc;
            double sourceRMS = 1.773; /*Vpp:5V, Vamp:2.5V, Vrms:1.773 */
        ...
        public void CreateWave()
        {
            ...
            double[]  values = new double[cycle * (int)((Math.PI*2)/resolution)];

            int index = 0;

            for (int c = 0; c < cycle; c++)
            {
                for (double ph_step = 0; ph_step < 2*Math.PI; ph_step += resolution)
                {
                    values[index++] = Math.Round(Math.Sin(ph_step + phase), decimals);
                }
            }
